...
export function RoleSwitcher({
  ...
  const canApplyFor = modesQuery.data?.can_apply_for ?? []
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="rounded-full bg-violet-100 px-2.5 py-1 text-xs font-semibold text-lokals-purple">{formatRoleLabel(active)} Mode</span>
      ...
      {open ? (
        <div className="absolute right-0 top-[calc(100%+0.5rem)] z-40 min-w-56 rounded-[20px] border border-lokals-border bg-white p-2 shadow-soft-lg">
          <p className="px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-lokals-muted">Switch mode</p>
          <div className="space-y-1">
            {visible.map((role) => {
              ...
              return (
                ...
                >
                  <span>{formatRoleLabel(role)} Mode</span>
            ...
            {pending.map((application) => (
              <div key={application.id} className="flex items-center justify-between rounded-2xl border border-lokals-border bg-slate-50 px-3 py-3 text-sm">
                <span className="font-medium text-lokals-charcoal">{formatRoleLabel(application.requested_role)} Mode</span>
                <span className="text-xs text-lokals-muted">{application.status.replaceAll('_', ' ')}</span>
              </div>
            ))}
            {canApplyFor.length ? (
              <>
                <p className="px-3 pt-3 text-xs font-semibold uppercase tracking-[0.2em] text-lokals-muted">Locked modes</p>
                <div className="space-y-1">
                  {canApplyFor.slice(0, 4).map((role) => (
                    <div key={String(role)} className="flex items-center justify-between rounded-2xl border border-lokals-border bg-white px-3 py-3 text-sm">
                      <span className="font-medium text-lokals-charcoal">{formatRoleLabel(role)} Mode</span>
                      <span className="text-xs text-lokals-muted">Apply</span>
                    </div>
                  ))}
                </div>
                <Link
                  ...
                  className="mt-2 block rounded-2xl bg-lokals-green px-3 py-3 text-center text-sm font-semibold text-white"
                >
                  Apply for role
                </Link>
              </>
