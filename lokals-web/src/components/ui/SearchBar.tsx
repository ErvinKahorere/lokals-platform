  ...
  return (
    <div className="relative">
      ...
      {(focused || recent.length > 0) && (shortcuts.length > 0 || recent.length > 0 || visibleSuggestions.length > 0) ? (
        <div className="glass-surface absolute left-0 right-0 top-[calc(100%+0.5rem)] z-20 rounded-[20px] border border-lokals-border p-3 shadow-soft-lg">
          ...
          {recent.length > 0 ? (
            <div className="mb-3">
              ...
              <div className="flex flex-wrap gap-2">
                {recent.map((item) => (
                  <button key={item} type="button" onMouseDown={() => selectValue(item)} className="inline-flex items-center gap-1 rounded-full bg-lokals-border/20 px-3 py-2 text-xs font-semibold text-lokals-charcoal">
