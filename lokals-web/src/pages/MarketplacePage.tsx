...
import { Clock3, MapPin, ShieldCheck, Sparkles, Store, TrendingUp, Warehouse } from 'lucide-react'
import { Link } from 'react-router-dom'
import { Button, EmptyState, Input, ListingCard, PageHeader, ProductCard, QueryState, SearchBar, SectionCard, StatusBadge, TextArea } from '../components/Ui'
...
export function MarketplacePage() {
  ...
  const recentItems = [...commerceProducts].sort((a, b) => Number(b.id) - Number(a.id)).slice(0, 8)
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[32px] border border-lokals-border bg-[radial-gradient(circle_at_top_left,_rgba(22,163,74,0.14),_transparent_32%),linear-gradient(135deg,#ffffff,#f8fafc,#f5f3ff)] p-6 shadow-card">
        <PageHeader
          ...
          title="Find local deals with confidence"
          description="Browse Okahandja products, local sellers, and trusted nearby listings in one calm commerce panel."
          actions={
            <div className="w-full md:w-[360px]">
              <SearchBar
                ...
                suggestions={['Electronics nearby', 'Furniture', 'Verified sellers', 'Fast delivery']}
                shortcuts={[{ label: 'Fast delivery', value: 'fast delivery' }, { label: 'Popular in Okahandja', value: 'popular okahandja' }]}
                placeholder="Search products, sellers, and local deals..."
              />
            </div>
          }
        />
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <div className="rounded-[24px] bg-white/80 p-4 backdrop-blur">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-lokals-muted">Location</p>
            <p className="mt-2 inline-flex items-center gap-2 text-sm font-semibold text-lokals-charcoal"><MapPin className="h-4 w-4 text-lokals-green" />Okahandja, nearby</p>
          </div>
          <div className="rounded-[24px] bg-white/80 p-4 backdrop-blur">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-lokals-muted">Safe local buying</p>
            <p className="mt-2 text-sm text-lokals-charcoal">Use verified sellers where possible and confirm items before payment.</p>
          </div>
          <div className="rounded-[24px] bg-white/80 p-4 backdrop-blur">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-lokals-muted">Live supply</p>
            <p className="mt-2 text-sm text-lokals-charcoal">{commerceProducts.length} active products and {legacyListings.length} local listings in the current view.</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-lokals-muted">
          <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 ring-1 ring-black/5"><ShieldCheck className="h-4 w-4 text-lokals-green" />Verified sellers surface first when available</span>
          <span className="rounded-full bg-white px-3 py-2 ring-1 ring-black/5">Call, WhatsApp, and view details stay one tap away</span>
        </div>
      </section>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <SectionCard className="bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-lokals-charcoal">Featured sellers</h2>
              <p className="text-sm text-lokals-muted">Verified, active, and easy to contact when available.</p>
            </div>
          </div>
      ...
      <SectionCard className="bg-white">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-lokals-purple">Featured Picks</p>
            <h2 className="mt-1 text-xl font-semibold text-lokals-charcoal">Featured products and verified sellers</h2>
          </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <SectionCard className="bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-lokals-charcoal">Recent products near you</h2>
              <p className="text-sm text-lokals-muted">Fresh local items with clear price, seller, and location context.</p>
            </div>
          </div>
        ...
        <SectionCard className="bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-lokals-charcoal">Recent community listings</h2>
              <p className="text-sm text-lokals-muted">Non-store marketplace posts for local supply, services, and quick discoveries.</p>
            </div>
          </div>
      ...
      <SectionCard className="bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-lokals-charcoal">Post a local listing</h2>
            <p className="text-sm text-lokals-muted">Keep the original publish flow available for non-store local listings and quick discoveries.</p>
          </div>
        </div>
