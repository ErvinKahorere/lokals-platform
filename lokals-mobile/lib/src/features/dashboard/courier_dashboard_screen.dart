...
class _CourierDashboardScreenState extends ConsumerState<CourierDashboardScreen> {
  ...
  Future<void> _toggleAvailability(bool isOnline) async {
    ...
    try {
      await ref.read(discoveryRepositoryProvider).updateCourierAvailability(!isOnline);
  ...
  Future<void> _handleDeliveryAction(int deliveryId, String action) async {
    ...
    try {
      await ref.read(discoveryRepositoryProvider).courierDeliveryAction(
    ...
    return LokalsShell(
      ...
      child: dashboard.when(
        data: (data) {
          ...
          final availableDeliveries = ((data['available_deliveries'] as List?) ?? const [])
              .map((item) => Map<String, dynamic>.from(item as Map))
              .toList();
          final deliveryHistory = ((data['delivery_history'] as List?) ?? const [])
              .map((item) => Map<String, dynamic>.from(item as Map))
              .toList();
          final activeDelivery = data['active_delivery'] is Map ? Map<String, dynamic>.from(data['active_delivery'] as Map) : null;
          ...
          return DashboardScaffold(
            ...
            quickActions: [
              ...
              DashboardQuickActionTile(
                ...
                body: isOnline ? 'Pause new parcel matching for now.' : 'Become available for nearby delivery requests.',
            ...
            extraSections: [
              _StatusHintCard(
                title: 'Availability',
                body: isOnline ? 'You are visible for new parcel requests.' : 'You are offline and hidden from new parcel matching.',
              ...
              if (activeDelivery != null)
                _ActionSection(
                  ...
                  subtitle: 'Move the current parcel through its next handoff step.',
                  child: _TransportCard(
                    title: '${activeDelivery['pickup_location'] ?? activeDelivery['pickup_address'] ?? 'Pickup'} -> ${activeDelivery['dropoff_location'] ?? activeDelivery['dropoff_address'] ?? 'Drop-off'}',
                    body: '${activeDelivery['user']?['name'] ?? 'Resident'} | ${activeDelivery['status_label'] ?? activeDelivery['status'] ?? 'accepted'}',
                    actions: [
                      if (activeDelivery['status'] == 'accepted')
                        _CardAction(
                          label: _busyDeliveryId == activeDelivery['id'] ? 'Updating...' : 'Pickup confirmed',
                          onPressed: _busyDeliveryId == activeDelivery['id'] ? null : () => _handleDeliveryAction(activeDelivery['id'] as int, 'pickup-confirmed'),
                        ),
                      if (activeDelivery['status'] == 'pickup_confirmed')
                        _CardAction(
                          label: _busyDeliveryId == activeDelivery['id'] ? 'Updating...' : 'In transit',
                          onPressed: _busyDeliveryId == activeDelivery['id'] ? null : () => _handleDeliveryAction(activeDelivery['id'] as int, 'in-transit'),
                        ),
                      if (activeDelivery['status'] == 'in_transit')
                        _CardAction(
                          label: _busyDeliveryId == activeDelivery['id'] ? 'Updating...' : 'Delivered',
                          onPressed: _busyDeliveryId == activeDelivery['id'] ? null : () => _handleDeliveryAction(activeDelivery['id'] as int, 'delivered'),
                        ),
                    ],
                  ),
                ),
                    ...
                    : Column(
                        children: availableDeliveries.take(5).map((item) {
                          ...
                          return Padding(
                            ...
                            child: _TransportCard(
                              title: '${item['pickup_location'] ?? item['pickup_address'] ?? 'Pickup'} -> ${item['dropoff_location'] ?? item['dropoff_address'] ?? 'Drop-off'}',
                              body: '${item['user']?['name'] ?? 'Resident'} | ${item['parcel_size'] ?? 'Parcel'} | N\$ ${item['estimated_price'] ?? '0'}',
                              actions: [
                                _CardAction(
                                  label: isBusy ? 'Updating...' : 'Accept',
                                  onPressed: isBusy ? null : () => _handleDeliveryAction(deliveryId, 'accept'),
                                ),
                                _CardAction(
                                  label: 'Decline',
                                  variant: AppButtonVariant.secondary,
                                  onPressed: isBusy ? null : () => _handleDeliveryAction(deliveryId, 'decline'),
                                ),
                              ],
                            ),
                          );
